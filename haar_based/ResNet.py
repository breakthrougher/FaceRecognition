...
train_transform = transforms.Compose([
    ...
    transforms.Resize(224),  # 适应ResNet输入尺寸
    transforms.ToTensor(),                       # 转换为Tensor
    transforms.Normalize(mean=[0.5], std=[0.5])  # 归一化，假设图像是灰度图
])
...
test_transform = transforms.Compose([
    ...
    transforms.Resize(224),  # 适应ResNet输入尺寸
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.5], std=[0.5])  # 测试集可以不做数据增强，主要做归一化
])
...
# 转换为 PyTorch Tensor 并调整形状 (N, 1, H, W)
...
# train_dataset = [(train_transform(img), label) for img, label in train_dataset]  # 应用增强

test_dataset = TensorDataset(X_test_tensor, y_test_tensor)
# test_dataset = [(test_transform(img), label) for img, label in test_dataset]  # 应用归一化
...
# 模型实例化
model = ResNetModel(num_classes=10)  # 假设有10个类别
